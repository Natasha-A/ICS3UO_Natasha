...
Attack Damage Outcomes - During Battle 
...
Type Advantages - Stronger Pokemon - Buring Battle, increase attack damage by half. HP amount Weaker Pokemon -
# decreases attack by half HP amount
...
# Allows to print characters one by one, in order to emulate fashion in game!
def delay_print(s):
    for c in s:
        sys.stdout.write(c)
        sys.stdout.flush()
    ...
    def selectPokemon(self):
        ...
        while checkPokemon:
            try:
                ...
                delay_print("Incorrect input. Please enter the # for one of the pokemons above.")
                checkPokemon = True
            except ValueError:
                delay_print("Incorrect value type. Please enter one of the numbers above.")
...
class Enemy(Pokemon):
    ...
    def randomPokemonSelected(self):
        ...
        # format title
        enemyPrintStatement = "Pokemon Trainer RED sent out " + str(nameOfPokemon) + "!\n"
        print("\n")
        delay_print(enemyPrintStatement)
        time.sleep(0.7)
...
def typeAdvantage(sidedObjectKind, opposingObjectKind, damageAmount):
    ...
    # calcuates new damage based on type advantage
    return damageAmount
...
def attackOutcome(attackChoice, pokemonSidedObject, pokemonOpposedObject):  # eg when CPU's Turn: "enemyObject" = \
    ...
    # High Range Damage:
        ...
        if attackDamage >= 30:
            printline = "It's extremely effective!\n"
            delay_print(printline)
        elif attackDamage >= 20:
            printline = "It's super effective!\n"
            delay_print(printline)

        else:
            printline = "It's not very effective...\n"
            delay_print(printline)
        time.sleep(1)
        ...
        print(opposingPokemon.name, "health has been decreased by", attackDamage, "HP and is now at", opposingPokemon.health, "HP.")
        time.sleep(1)
...
def playGame(playerObject, enemyObject):
    ...
    while newRound:
        ...
        time.sleep(1.5)
        delay_print("Pokemon Trainer RED wants to battle!\n")
        time.sleep(1)
