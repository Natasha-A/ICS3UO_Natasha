...
message = StringVar() #resets every time btnPress()
...
message = StringVar() #resets every time btnPress()
message.set("Welcome to the Game!")
...
# label widget
label = tk.Label(root, textvariable = message) #setting to equation -- changes as it updates using
...
def playGame(playerObject, enemyObject):
    ...
    while newRound:
        ...
        time.sleep(3)
        root.after(2000, message.set("Pokemon Trainer RED wants to battle!"))
        time.sleep(1)
        root.mainloop()
